...
exports.getData = async (req, res) => {
    let countData = await UserModel.find().count();
    await UserModel.find(
        (err, result) => {
            if (err) { res.status(400).send(err); }
            else { res.status(200).send({TotalData:countData, data: result})}
        }).sort({ "id": 1 });
};
...
exports.postData = async (req, res) => {
    ...
    const {fname,lname,user,email,phone,gender,dob,states,tnc,city,password} = req.body
    let id = Math.floor(Math.random() * 100)
    const temp_api = new UserModel({fname,lname,user,email,phone,gender,dob,states,tnc,city,password,id});
    try {
        await temp_api.save();
        res.send(req.body);
    } catch (err) {
...
exports.getuser = async (req, res) => {
    ...
    debugger
    await UserModel.findOne({ user }).then(profile => {
...
let string = new RegExp(req.params.string , 'i')
